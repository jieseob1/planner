...
4. Docker 이미지의 OS·ARM64·revision label을 검사하고 kind의 모든 기존 노드에 import합니다. import한 manifest와 CRI 이미지의 config digest를 GHCR 원본과 비교합니다. kind가 `import-날짜` OCI index를 Pod imageID로 반환하면 index → 단일 manifest → config까지 추적한 뒤에만 같은 이미지로 인정합니다.
