...
describe('LandingScreen', () => {
  it('explains the plan-to-action value and exposes the real product evidence', () => {
    ...
    expect(screen.getByRole('img', { name: 'Goals to Today 오늘 실행 화면' })).toBeInTheDocument();
    expect(screen.getByRole('img', { name: 'Goals to Today 목표와 지표 화면' })).toBeInTheDocument();
    expect(screen.getByRole('img', { name: 'Goals to Today 주간 계획 화면' })).toBeInTheDocument();
    expect(screen.getByRole('img', { name: 'Goals to Today 주간 회고 화면' })).toBeInTheDocument();
  });
