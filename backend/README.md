...
Java 25 / Spring Boot 4.1.1 기반의 stateless API입니다. PostgreSQL이 planner, revision, plan lifecycle, audit, OAuth, job lease, notification과 rate-limit 상태의 단일 기준입니다.
...
백엔드만 실행하려면 PostgreSQL 17을 준비하고 local-auth profile을 명시합니다.

```bash
export SPRING_DATASOURCE_URL=jdbc:postgresql://localhost:5432/nowline
...
OAuth state와 PKCE verifier는 10분 만료되는 일회성 DB row입니다. refresh token은 AES-256-GCM으로 암호화하며 access token은 저장하지 않습니다. worker는 PostgreSQL `FOR UPDATE SKIP LOCKED` lease로 여러 replica에서 job 하나를 한 번만 소유합니다.
...
Testcontainers PostgreSQL에서 migration, auth failure, tenant 격리, lifecycle/audit, concurrency/idempotency, calendar sync, preferences/export/deletion, health/metrics를 확인합니다. 루트의 `npm run verify:e2e`는 실제 Compose HTTP 경로, `npm run verify:k8s:runtime`은 두 Pod의 concurrent ETag 경로를 검증합니다.
